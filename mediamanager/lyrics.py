...
class ChartLyrics():
    ...
    tag_lyric, tag_lyric_id, tag_lyric_checksum  = ['{http://api.chartlyrics.com/}%s' % x for x in tags]
    ...
    def get(self, info):
        ...
        lyric_id, lyric_check_sum = [quote(info.get(x)) for x in ['artist', 'title']]
    ...
    def search(self, info):
        ...
        """
        ret = dict()
        artist, song = [quote(info.get(x).lower()) for x in ['artist', 'title']]
